...
final class SquirrelView: NSView {
  ...
  var backgroundPath: CGPath?  // 主面板边框路径（供激光蛇使用）
  ...
  override func draw(_ dirtyRect: NSRect) {
    ...
    let localBackgroundPath = drawSmoothLines(rectVertex(of: backgroundRect), straightCorner: Set(), alpha: 0.3 * theme.cornerRadius, beta: 1.4 * theme.cornerRadius)
    backgroundPath = localBackgroundPath  // 保存供激光蛇使用

    // 保存效果层（霓虹边框等），避免被清除
    let effectLayers = self.layer?.sublayers?.filter { sublayer in
      // 通过 name 属性识别效果层容器
      sublayer.name == "neonBorderContainer" || sublayer.name == "effectsContainer"
    }

    self.layer?.sublayers = nil

    // 恢复效果层
    if let effectLayers = effectLayers {
      for effectLayer in effectLayers {
        self.layer?.addSublayer(effectLayer)
      }
    }
    let backPath = localBackgroundPath?.mutableCopy()
    ...
    let panelLayerMask = shapeFromPath(path: localBackgroundPath)
    ...
    if let color = theme.preeditBackgroundColor, let path = preeditPath {
      ...
      let maskPath = localBackgroundPath?.mutableCopy()
    ...
    if theme.borderLineWidth > 0, let color = theme.borderColor {
      let borderLayer = shapeFromPath(path: localBackgroundPath)
    ...
    if let color = theme.highlightedBackColor, let path = highlightedPath {
      ...
      if theme.shadowSize > 0 {
        ...
        let outerPath = localBackgroundPath?.mutableCopy()
    ...
    panelPath.addPath(localBackgroundPath!, transform: panelLayer.affineTransform().scaledBy(x: 1, y: -1).translatedBy(x: 0, y: -dirtyRect.height))
    ...
    // 通知绘制完成，以便激光蛇边框可以安全地初始化
    NotificationCenter.default.post(name: NSNotification.Name("SquirrelViewDidFinishDrawing"), object: self)
  }
